...
The release ZIP is about 1.6 MB and installs the three locked runtime packages (CodeIgniter framework, Laminas Escaper, and PSR Log) in a separate `--no-dev` Composer step during packaging. The package manifest and SHA-256 checks pass. Local Apache HTTP checks with the ZIP at its document root and below `/support/` also passed, including eight ticket cases, private-area blocking, and versioned CSS loading. Public HTTPS deployment and a real Cafe24 host remain unverified.
...
The repository root is the source and development root. The source web document root is `public/`; application code, configuration, tests, and packaging tools stay above it. The completed release ZIP has a different shape: its extracted root contains `index.php`, `.htaccess`, `assets/`, and `_supportk/`, and that extracted root is the package document root. The package entry point loads the active release from `_supportk/releases/<version>/`.
...
(cd dist && sha256sum -c support-k-0.1.0-alpha.1.zip.sha256)
```

The build uses the runtime packages from `composer.lock`, excludes development dependencies from the release install, writes a manifest for every packaged file, creates protected runtime directories, and emits a SHA-256 sidecar. The verifier checks traversal and symlink safety, the active release metadata, required runtime directories, the lock-based component list, the file manifest, and forbidden configuration, log, test-runtime, and data files.
