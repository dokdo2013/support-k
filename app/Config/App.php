...
class App extends BaseConfig
{
    ...
    {
        parent::__construct();
        $runtime = \App\Services\Installation\Runtime::read();
        if (isset($runtime['base_url'])) {
            $this->baseURL = $runtime['base_url'];
        } elseif (!is_cli() && !getenv('app.baseURL')) {
            $host = $_SERVER['HTTP_HOST'] ?? 'localhost';
            if (!preg_match('/^[a-zA-Z0-9.\-]+(?::[0-9]{1,5})?$/D', $host)) {
                $host = 'localhost';
            }
            $path = rtrim(str_replace('\\', '/', dirname($_SERVER['SCRIPT_NAME'] ?? '/index.php')), '/.');
            $this->baseURL = ((!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https://' : 'http://') . $host . $path . '/';
        }
    }
